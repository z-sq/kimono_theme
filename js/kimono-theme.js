...
(function ($) {
  $(document).ready(function () {
    ...
    $(".tourism_consldridx_length").text($(".tourism_consldr_item").length);
    // tourismsldr.on("move", function () {
    //   $(".tourism_consldridx_active").text(tourismsldr.index + 1);
    // });
  });
  ...
  $(window).on("load scroll resize", function () {
    ...
    if (_ww > 767) {
      ...
      if ($(window).height() < $(".header_logo").height() + _hdr_menu_h) {
        $(".header_menu_wrp").css({ overflow: "scroll", height: _hdr_menu_h });
      } else {
  ...
  // 言語メニュー
  $wpml_menu = $(".wpml-ls-sub-menu");
  $(".wpml-ls-native").on("click", function () {
    if ($(this).hasClass("active")) {
      //閉じる
      $(this).removeClass("active");
      $wpml_menu.removeClass("show");
    } else {
      //開く
      $(this).addClass("active");
      $wpml_menu.addClass("show");
    }
  });
})(jQuery);
